...
* Date: The date when the 10 fly traps were __collected__, each traps are out for a week
...
Note: the data includes lots of NA's which are bad for data analysis. 
...
```{r, echo = F, include=F}
#this allows you to run any amount of code without printing; we could try and surpress warnings, or just make a copy to not print warnings 
library("tidyr")
library("dplyr")
library("lubridate")
```
